...
def test_sell():
        ...
        TouchAction(driver).long_press(a, None, None, 3000).release().perform()

        driver.find_element(MobileBy.ID, 'online.paycash.app:id/tv_mainnet_preprod').click()
        for i in range(4):
            driver.find_element(MobileBy.ID, 'online.paycash.app:id/cv_0').click()'''
        driver.find_element(MobileBy.ID, 'online.paycash.app:id/navigation_pay_cash_sell').click()
        driver.find_element(MobileBy.ID, 'online.paycash.app:id/tv_create_order').click()
        driver.find_element(MobileBy.ID, 'online.paycash.app:id/iv_bank_card').click()
        driver.find_element(MobileBy.ID, 'online.paycash.app:id/tv_action').click()
        driver.find_element(MobileBy.ID, 'online.paycash.app:id/et_card_number').send_keys('4111111111111111')
        driver.find_element(MobileBy.ID, 'online.paycash.app:id/toolbar_title').click()
        driver.find_element(MobileBy.ID, 'online.paycash.app:id/btn_save').click()
        driver.find_element(MobileBy.ID, 'online.paycash.app:id/toolbar_left_button').click()
        driver.find_element(MobileBy.ANDROID_UIAUTOMATOR, 'text("Enter the withdrawal amount")').send_keys('0.1')
        driver.find_element(MobileBy.ANDROID_UIAUTOMATOR, 'text("Create order")').click()
